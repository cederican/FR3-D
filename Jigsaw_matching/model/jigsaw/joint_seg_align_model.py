...
class JointSegmentationAlignmentModel(MatchingBaseModel):
    ...
    def forward(self, data_dict):
        ...
        if critical_feats.shape[1] == 0:
            # if no critical points, return empty
            out_dict.update(
                {
                    "ds_mat": torch.zeros(B, 0, 0, device=self.device),
                    "perm_mat": torch.zeros(B, 0, 0, device=self.device),
                }
            )
            return out_dict
        else:
    ...
    def _loss_function(self, data_dict, out_dict={}):
        ...
        N_ = torch.max(n_critical_pcs_sum)
        if N_ == 0:
            return { "loss": torch.tensor(0.0, device=self.device)}, torch.zeros(
                B, N_, N_, device=self.device
            )
        with torch.no_grad():
