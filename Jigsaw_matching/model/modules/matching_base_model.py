...
class MatchingBaseModel(pytorch_lightning.LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        func = torch.tensor if isinstance(self.validation_outputs[0]['batch_size'], int) else torch.stack
        batch_sizes = func([output.pop('batch_size') for output in self.validation_outputs]).type_as(self.validation_outputs[0]['loss'])
        losses = {f'val/{k}': torch.stack([output[k] for output in self.validation_outputs]).reshape(-1)
                  for k in self.validation_outputs[0].keys()}
        avg_loss = {k: (v * batch_sizes).sum() / batch_sizes.sum() for k, v in losses.items()}
    ...
    def test_step(self, data_dict, batch_idx):
        # save_dir = "/home/datasets/Breaking-Bad-Dataset.synthezised.bones/advancedTibiaHead/matching_data_finetune"
        # os.makedirs(save_dir, exist_ok=True)
        # save_path = os.path.join(save_dir, f"{data_dict['data_id'].item()}.npz")
    ...
    def on_test_epoch_end(self):
        ...
        if isinstance(self.test_outputs[0]['batch_size'], int):
        ...
        batch_sizes = func_bs([output.pop('batch_size') for output in self.test_outputs]).type_as(self.test_outputs[0]['loss'])

        losses = {
            f'test/{k}': func_loss([output[k] for output in self.test_outputs])
    ...
    def _cus_vis(self, data_dict, pred_trans, pred_rot, gt_trans, gt_rot, part_acc):
        print(f"Processing data with ID: {data_dict['data_id'][0].item()}---")
        ...
        for i in range(B):
            save_dir = os.path.join("inference_artifact", str(data_dict["data_id"][i].item()))
            os.makedirs(save_dir, exist_ok=True)
    ...
    def transformation_loss(self, data_dict, out_dict):
        gt_perm = out_dict['gt_perm'].cpu().numpy()  # [B, N_, N_]
    ...
    def compute_global_transformation(self, n_critical_pcs, match_mat, gt_match_mat, gt_pcs,
        ...
        for b in range(B):
            ...
            gt_edges, edges, transformations, uncertainty = [], [], [], []

            gt_corr_list, corr_list = [], []
            gt_pc_list = []
            ...
            for idx1, idx2 in lexico_iter(np.arange(n_valid[b])): # All combinations of pieces
                ...
                gt_mat = gt_match_mat[b, cri_st1:cri_ed1, cri_st2:cri_ed2]
                gt_mat_s = np.sum(gt_mat).astype(np.int32)
                gt_mat2 = gt_match_mat[b, cri_st2:cri_ed2, cri_st1:cri_ed1]
                gt_mat_s2 = np.sum(gt_mat2).astype(np.int32)

                if gt_mat_s < gt_mat_s2:
                    gt_mat = gt_mat2.transpose(1, 0)
                    gt_mat_s = gt_mat_s2
                if n_valid[b] > 2 and gt_mat_s == 0 and sum_full_matched > 0:
                    continue
                if np.count_nonzero(gt_mat) < 3:
                    continue
                ...
                if critical_pcs_idx is not None:
                    ...
                    _ , gt_corr = get_trans_from_mat(critical_pcs_src, critical_pcs_tgt, gt_mat)

                    gt_corr_list.append(gt_corr)
                    gt_edges.append(np.array([idx2, idx1]))
    ...
    def loss_function(self, data_dict, mode):
        ...
        loss_dict, gt_perm = self._loss_function(data_dict, out_dict)
        out_dict['gt_perm'] = gt_perm
    ...
    def forward_pass(self, data_dict, mode):
        loss_dict = self.loss_function(data_dict, mode=mode)
    ...
    def _save_data(self, edges, gt_edges, corr_list, gt_corr_list, gt_pcs, critical_pcs_idx, n_pcs, n_critical_pcs, data_id):
        """
        ...
        data = {
            "edges": edges,
            "gt_edges": gt_edges,
            "correspondence": np.array(corr_list, dtype=object),
            "gt_correspondence": np.array(gt_corr_list, dtype=object),
            "gt_pcs": gt_pcs,
        ...
        np.savez(save_path, **data, allow_pickle=True)
