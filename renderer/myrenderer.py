...
class MyRenderer:
    def __init__(self, cfg):
        ...
        self.use_GPU = True
        self.scale = (1, 1, 1)
        self.jigsaw = cfg.renderer.jigsaw
        self._init_param()
    ...
    def _init_param(self):
        ...
        self.location_offset = Vector(((0, 0, 0)))
    ...
    def generate_camera_positions(self, num_frames=10, radius=3.16, height=1.5):
        camera_positions = []
        ...
        for i in range(num_frames):
            ...
            camera_positions.append([x.item(), y.item(), z])
    ...
    def sample_data_files(self):
        ...
        """
        if self.cfg.denoiser.sampling and not self.cfg.denoiser.save_best_flag:
            self.inference_path = os.path.join(self.inference_path, str(self.cfg.idx))
            all_files = [f for f in os.listdir(self.inference_path) if os.path.isdir(os.path.join(self.inference_path, f))]
        else:
            all_files = natsorted(
                [f for f in os.listdir(self.inference_path) if os.path.isdir(os.path.join(self.inference_path, f))]
            )
        ...
        if self.num_samples != -1:
            ...
            data_list = ['9', '10'] #['13', '14', '189', '221', '232', '875', '962', '1022', '1112', '1531', '2428'] #['187', '396', '666', '952', '953', '1003', '1139', '1378', '1393', '2702', '1408']
        return data_list
    ...
    def load_transformation_data(self, file):
        ...
        init_pose = np.load(f"{inference_data_path}/init_pose.npy") if not self.jigsaw else None
    ...
    def generate_artificial_rotations(self, transformation, num_frames=36):
        """
        Generate artificial transformations for rotating the object.
        These will be applied to the object to show different views (360-degree rotation).
        """
        rotations = []
        end_transformation = np.zeros((num_frames, transformation.shape[1], transformation.shape[2]))
        for i in range(transformation.shape[1]):
            end_transformation[:, i, :3] = transformation[transformation.shape[0]-1, i, :3]
            for j in range(num_frames):
                end_transformation[j, i, 3:] = R.from_euler('z', j * 360 / num_frames, degrees=True).as_quat()
        return end_transformation
    ...
    def compute_final_transformation(self, init_pose, gt_transformation, transformation):
        ...
        if init_pose is not None:
            ...
            final_transformation = trans_mat3 @ rot_mat3 @ rot_mat2 @ trans_mat2
        return final_transformation
    ...
    def save_img(self, parts, gt_transformatoin, transformation, init_pose, save_path, frame=None):
        self.render_parts(parts, gt_transformatoin, transformation, init_pose, frame)
