...
def get_final_pose_pts_dynamic(pts, n_pcs, pred_trans, pred_rots, num_parts, nodes=None):
    """
    ...
    for i in range(num_parts[0].item()):
        ...
        c_pivot = i if nodes == None else nodes[i]["pivot"]
...
def node_merge_valids_check(edges, ref_part, nodes):
    ref_part_idx = torch.where(ref_part)[0]
    idx1 = edges[0]
    idx2 = edges[1]
    ...
    if ref_part[pivot_idx1] or ref_part[pivot_idx2]: # Any of part is reference part
        return False
...
def get_next_available_indexed_dir(base_dir):
    i = 0
    while True:
