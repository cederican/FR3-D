...
import hydra
from FR3D.vqvae.dataset.dataset import build_geometry_dataloader, build_geometry_dataloader_real
import os
import numpy as np
...
def main(cfg):
    ...
    def save_data(data_type):
        ...
        if not data_type == 'real':
            loader = build_geometry_dataloader(cfg, data_type, start_id)
        else:
            loader = build_geometry_dataloader_real(cfg, data_type, start_id)
